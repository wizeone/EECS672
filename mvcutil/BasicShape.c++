...
BasicShape::BasicShape() :
	pointCoords(NULL), normals(NULL), textureCoords(NULL), nPoints(0),
	drawArraysCallData(NULL), nDrawArraysCalls(0),
	indexLists(NULL), nIndexLists(0), nIndexListsConstructed(0)
{
	...
	int firstAtBottom, int firstAtTop)
{
	...
	drawArraysCallData[0].usePerVertexTexCoords = (textureCoords != NULL);
...
void BasicShape::makeIndexLists(int nPointsAroundSide, int nPointsAlongAxis)
{
	...
	for (int i=1 ; i<nPointsAlongAxis ; i++) // for each index list:
	{
		...
		indexLists[nIndexListsConstructed].usePerVertexTexCoords = (textureCoords != NULL);
