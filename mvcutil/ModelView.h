...
// 2. Methods "handleCommand", "handleFunctionKey", and "handleSpecialKey" are
//    given default (empty) implementations. Subclasses need only override if
...
public:
	...
	virtual bool handleCommand(unsigned char key, double ldsX, double ldsY)
		{ return true; }
	virtual bool handleFunctionKey(int whichFunctionKey,
		double ldsX, double ldsY, int mods) { return true; }
	virtual bool handleSpecialKey(Controller::SpecialKey key,
		double ldsX, double ldsY, int mods) { return true; }
